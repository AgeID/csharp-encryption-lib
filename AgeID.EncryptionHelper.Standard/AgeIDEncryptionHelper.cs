	...
	public static class AgeIDEncryptionHelper
    {
		...
		public static string Decrypt (string value, string clientSecret, APIVersion apiVersion = APIVersion.V2)
        {
            try
            {
                ...
                if (Encoding.UTF8.GetBytes(saltString).Length < SALT_LENGTH_BYTES)
					throw new Exception("The payload is invalid.");
        ...
        private static byte[] AESDecryptBytes (byte[] cryptBytes, byte[] passBytes, byte[] saltBytes, APIVersion apiVersion = APIVersion.V2)
        {
        ...
        private static byte[] AESEncryptBytes (byte[] clearBytes, byte[] passBytes, byte[] saltBytes, APIVersion apiVersion = APIVersion.V2)
        {
